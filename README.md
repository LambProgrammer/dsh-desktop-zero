...
1. 下载并运行安装版或便携版。
2. 首次运行需要**自行配置 DeepSeek API Key**：在打开的 DSH 界面中进入设置，填入你的 API Key（可从 DeepSeek 开放平台获取）。
3. 开始使用。
...
- **安装包体积很大？**
  正常现象。应用内置了完整的 Node.js 运行时和 DSH 全套依赖（仅便携版单个 exe 约 100~300MB），这是"零依赖开箱即用"的代价。
...
- 主进程通过 `ELECTRON_RUN_AS_NODE` 模式以内置 Node 启动 DSH web 服务，渲染进程用 iframe 承载界面，启动时自动分配空闲端口
